...
const registerUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { name, email, password, role, specialty, location, experience, availabilitySlots } = req.body;
    try {
        const hashedPassword = yield bcryptjs_1.default.hash(password, 10);
        const newUser = new User_1.default({ name, email, password: hashedPassword, role });
        yield newUser.save();
        if (role === 'doctor') {
            const newDoctor = new Doctor_1.default({ name, specialty, location, experience, availabilitySlots });
